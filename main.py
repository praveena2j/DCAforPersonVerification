...
if args.train == True:
	while args.epoch < args.max_epoch:
		...
		if args.epoch % args.test_step == 0:
			s.save_parameters(model_save_path_a + "/model_%04d.model"%args.epoch, 'A')
			s.save_parameters(model_save_path_v + "/model_%04d.model"%args.epoch, 'V')
			s.save_parameters(model_save_path_av + "/model_%04d.model"%args.epoch, 'AV')
			s.eval_network(args)
		args.epoch += 1
	quit()
