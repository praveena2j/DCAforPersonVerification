...
class trainer(nn.Module):
	...
	def save_parameters(self, path, modality):
		...
		if modality == 'AV':
			model = OrderedDict(list(self.fusion_model.state_dict().items()) + list(self.speaker_face_loss.state_dict().items()))
			...
			if modality == 'AV':				
				if ('speaker_encoder.' not in name) and ('speaker_loss.' not in name):
					if ('face_encoder.' not in name) and ('face_loss.' not in name):
						if ('fusion_model.' not in name) and ('speaker_face_loss.' not in name):
							if name == 'weight':
								name = 'speaker_face_loss.' + name
							else:
								name = 'fusion_model.' + name
						self_state[name].copy_(param)
